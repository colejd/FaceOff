...
namespace
{
    ...
    public:
        ...
        void set(const float sigma_s, const float sigma_r, const int tree_height, const int num_pca_iterations);
    ...
    inline void ensureSizeIsEnough(cv::Size size, int type, Mat& m)
    {
    ...
    template <typename T>
    inline void ensureSizeIsEnough(cv::Size size, Mat_<T>& m)
    {
     ...
     */
    void AdaptiveManifoldFilterImpl::set(const float sigma_s, const float sigma_r, const int tree_height, const int num_pca_iterations){
        sigma_s_ = sigma_s;
        sigma_r_ = sigma_r;
        tree_height_=tree_height;
        num_pca_iterations_=num_pca_iterations;
    }
    ...
    void calcEta(const Mat_<Point3f>& src_joint_f, const Mat_<float>& theta, const Mat_<uchar>& cluster, Mat_<Point3f>& dst, float sigma_s, float df, Buf& buf)
    {
        ...
        const cv::Size nsz = cv::Size(saturate_cast<int>(buf.mul.cols * (1.0 / df)), saturate_cast<int>(buf.mul.rows * (1.0 / df)));

        ensureSizeIsEnough(nsz, buf.numerator);
        resize(buf.mul, buf.numerator, cv::Size(), 1.0 / df, 1.0 / df);

        ensureSizeIsEnough(nsz, buf.denominator);
        resize(buf.theta_masked, buf.denominator, cv::Size(), 1.0 / df, 1.0 / df);
    ...
    void AdaptiveManifoldFilterImpl::buildManifoldsAndPerformFiltering(const Mat_<Point3f>& eta_k, const Mat_<uchar>& cluster_k, int current_tree_level)
    {
        ...
        if (eta_k.rows == src_joint_f_.rows)
        {
            ...
            const cv::Size nsz = cv::Size(saturate_cast<int>(eta_k.cols * (1.0 / df)), saturate_cast<int>(eta_k.rows * (1.0 / df)));
            ensureSizeIsEnough(nsz, buf_.eta_k_small);
            resize(eta_k, buf_.eta_k_small, cv::Size(), 1.0 / df, 1.0 / df);
        }
        else
        {
